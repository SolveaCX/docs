...
Everyone starts with a key and one request. After that, jump to what you are building.

<CardGroup cols={2}>
  <Card title="Write code" icon="code" href="/guides/openai-sdk">
    Use the OpenAI or Anthropic SDK, or call the REST API directly.
  </Card>
  <Card title="Generate images and video" icon="image" href="/guides/image-generation">
    Create images with `gpt-image-2`, or [generate video](/guides/video-generation) with Seedance and `MiniMax-H3`.
  </Card>
  <Card title="Plug into an AI tool" icon="terminal" href="/guides/claude-code">
    Point Claude Code, Codex CLI, or Codex Desktop at Flatkey.
  </Card>
  <Card title="Call your own functions" icon="wrench" href="/guides/text-generation#call-your-own-functions">
    Add tool calling and structured outputs to a request.
  </Card>
</CardGroup>
...
- cURL, Python 3.10+, Node.js 22+, or PowerShell
...
The Python and TypeScript examples reuse the `client` you created in **Send your first request**.
...
| `503 model_not_found` | The model ID is misspelled or unavailable | Check the [Model Directory](https://flatkey.ai/models) for the exact ID. Retrying does not help |
| `429` | The request exceeded a rate limit | Retry with exponential backoff and jitter |
| `500` or `503` | The provider or route is temporarily unavailable | Read `error.code` in the response body first. Retry with backoff and check [Model Health](https://flatkey.ai/status) |

<Note>
  A `503` can mean either a temporary outage or an unknown model ID. Read `error.code` in the response body before you retry.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Billing" icon="wallet" href="https://flatkey.ai/pricing">
