...
[CC Switch](https://ccswitch.io) is a third-party desktop configuration manager for coding assistants. It is not a Flatkey product, and Flatkey is not a built-in preset. This tutorial was verified with CC Switch v3.17.0. Labels may differ in newer versions.
...
| Default model | `gpt-5.4` (example). Confirm and copy the current exact ID from the Flatkey [Model Directory](/reference/model-list) before use. |
...
Model mapping affects only the entries shown in the Codex `/model` menu. Use **Fetch Models** if your CC Switch version provides it, then map the exact IDs you want to show. Even when an exact model ID is not mapped or visible, you can set it as the provider **Default Model** or pass it directly with `codex --model <id>`.
...
| Endpoint or connection error | Set the API endpoint exactly to `https://router.flatkey.ai/v1`. Check for a stale `OPENAI_BASE_URL` in the new shell. |
| Model not found or missing from `/model` | Copy the exact ID from the [Model Directory](/reference/model-list). Use **Fetch Models** and model mapping when available. Mapping affects only `/model`; set the exact ID as the provider **Default Model** or pass it with `codex --model <id>`. |
