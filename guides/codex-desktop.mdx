...
| Default Model | `gpt-5.6-sol` in the screenshot — verify a current exact ID in the [Flatkey Model Directory](/reference/model-list) |
...
Set the provider's **Default Model** to the exact Flatkey model ID you want to use. The screenshot shows `gpt-5.6-sol` as an example. Check the [Flatkey Model Directory](/reference/model-list) before choosing an ID.
...
| `Model not found`, custom model is hidden, or `/model` mapping is missing | Copy an exact ID from the [Model Directory](/reference/model-list), set it as **Default Model**, and use **Fetch Models** or model mapping |
